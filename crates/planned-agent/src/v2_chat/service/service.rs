...
use super::config::V2ChatConfig;
use crate::v2_chat::driver::driver_loop;
use super::event::{SubscriptionGuard, SubscriptionId, V2ChatEvent};
use crate::v2_chat::state::{resolve_ai_client, State};
...
#[derive(Clone)]
pub struct V2ChatService<PM: PromptManager + Send + Sync + 'static> {
    state: std::sync::Arc<State<PM>>,
}

impl<PM: PromptManager + Send + Sync + 'static> std::fmt::Debug for V2ChatService<PM> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("V2ChatService")
            .field("config", &*self.state.config.lock().unwrap())
            .finish()
    }
}

impl<PM: PromptManager + Send + Sync + 'static> V2ChatService<PM> {
    /// 通过 `AiManager` + 配置构造。
    pub fn new(
    ...
    pub fn from_ai_client(
        ...
        let state = std::sync::Arc::new(State {
            ...
            history: crate::v2_chat::state::History::new(),
            subscribers: crate::v2_chat::state::Subscribers::new(),
            cmd_tx,
            driver_rx: std::sync::Mutex::new(Some(cmd_rx)),
            driver_started: AtomicBool::new(false),
            run_state: std::sync::Mutex::new(crate::v2_chat::state::RunState::Idle),
    ...
    pub fn on_chat(&self, handler: impl Fn(V2ChatEvent) + Send + Sync + 'static) -> SubscriptionId {
    ...
    pub fn is_awaiting_user_action(&self) -> bool {
        matches!(
            *self.state.run_state.lock().unwrap(),
            crate::v2_chat::state::RunState::AwaitingUserAction
        )
    }
