//! v2 聊天配置
//!
//! [`V2ChatConfig`] 是 [`crate::v2_chat::V2ChatService`] 的纯数据配置，
//! 字段与 v1 [`crate::chat::ChatConfig`] 保持一致（不含任何子 agent 概念），
//! 通过 [`V2ChatConfig::default`] 获得保守默认后按需修改。
...
pub struct V2ChatConfig {
    ...
    /// - 解析：`V2ChatService` 通过注入的 `PromptManager::render(path, ctx)`
    ...
    /// `V2ChatEvent::Done`（`finish_reason` 为 `None`）。
    ...
    ///
    /// 子 agent 的 `V2ChatService` 是注册时创建、多次调用复用的，因此 run_id
    /// 由 runner 在每次 `start` 前通过 [`crate::v2_chat::V2ChatService::set_run_id`]
    /// 动态注入（子 agent 串行执行，复用安全）。
    pub run_id: Option<String>,
}

impl Default for V2ChatConfig {
    fn default() -> Self {
        Self {
            provider: None,
