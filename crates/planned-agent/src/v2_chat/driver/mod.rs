...
use crate::v2_chat::service::{SendOutcome, V2ChatEvent};
use crate::v2_chat::state::{Command, RunState, State};
use round::{run_conversation, ConversationOutcome, UIActionStrategy};

/// 常驻 driver task：串行消费 `Command`。
///
/// - `Weak<State>`：所有 `V2ChatService` 实例都已 drop 时退出，避免 `State`
    ...
    loop {
        ...
        match cmd {
            ...
            Command::Confirm {
                ...
                state.subscribers.emit(V2ChatEvent::Error(format!(
            ...
            Command::Resume {
                ...
                let Some(tool_call_id) = state.history.find_pending_ui_tool_call_id() else {
                    ...
                    state.subscribers.emit(V2ChatEvent::Error(msg.clone()));
    ...
    *state.run_state.lock().unwrap() = RunState::Idle;
    ...
    match result {
        Ok(ConversationOutcome::Completed) => {
            ...
            state.subscribers.emit(V2ChatEvent::Done {
        ...
        Err(e) => {
            ...
            state.subscribers.emit(V2ChatEvent::Error(e.to_string()));
