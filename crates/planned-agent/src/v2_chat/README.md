...
│   └── mod.rs    V2ChatSubAgentRunner / V2ChatSubAgentSession
├── tools/       工具相关
...
用户输入
  │
  ▼
V2ChatService::send_text("帮我分析代码")
  │  → Command::Send 入队
  │  → return SendTicket
  ▼
...
round.rs: bridge.needs_stream("data_expert") → true
  ...
  │       │
  │       ▼
  │     V2ChatSubAgentRunner::start(arguments, stream)
  │       │
...
- **显式启动**：`start_driver()` 必须在首次 `send` / `confirm_user_action` /
  ...
  子 agent 在 `V2ChatSubAgentRunner::start()` 内部调用。
...
| **子 agent 复用** | 子 agent 直接复用 V2ChatService，不重复实现 ReAct 循环 |
