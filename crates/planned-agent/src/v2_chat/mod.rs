...
//!   完整多轮 tool-call 循环；
//! - **事件订阅**：[`V2ChatService::on_chat`] 注册监听（多订阅者、可取消），
//!   事件类型 [`V2ChatEvent`] 复用 core [`ChatEvent`] 并补充
//!   `Done` / `Error` 生命周期信号；
//! - **UI 交互按 tool 消息回填**：[`V2ChatService::confirm_user_action`]
...
//! | 类型 | 模块 | 说明 |
//! |---|---|---|
//! | [`V2ChatService`] | `service` | 有状态聊天服务入口 |
//! | [`SendTicket`] | `service` | `send` 的完成凭证（可 `await`） |
//! | [`V2ChatEvent`] | `service` | 事件协议（core `ChatEvent` + Done/Error） |
//! | [`V2ChatConfig`] | `service` | 纯数据配置 |
//!
//! # 目录结构
//!
//! ```text
...
//! ├── service/    对外 API 层：V2ChatService / SendTicket / V2ChatConfig /
//! │               V2ChatEvent（只负责接口与入队，无后台逻辑）
...
pub use service::{SendTicket, SubscriptionGuard, SubscriptionId, V2ChatConfig, V2ChatEvent, V2ChatService};
pub use sub_agent::{V2ChatSubAgentRunner, V2ChatSubAgentSession};
