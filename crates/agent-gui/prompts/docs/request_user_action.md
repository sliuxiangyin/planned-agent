# request_user_action 工具规范

## 概述

`request_user_action` 用于向用户发起结构化 UI 交互。
一次调用携带 1..≤4 个**彼此独立**的并列问题；前端渲染成「逐题向导卡片」——一次只显示一题（右上角显示页码，如 `1/4`），用户逐题作答，到最后一题统一提交、整体一次回传。
调用后等待用户操作，不要自行假设用户选择。

调用参数：

| 参数 | 必填 | 说明 |
|------|------|------|
| message | ❌ | 可选：整批问题的引导文本，说明需要用户做什么决定 |
| questions | ✅ | 并列的问题列表（数组），1..≤4 个，彼此独立 |

调用 JSON：

```json
{
  "message": "可选：整批问题的引导文本",
  "questions": [
    {
      "header": "短标签（≤4字，同批内唯一）",
      "question": "问题全文，说明需要用户做什么决定",
      "multi": false,
      "allow_input": true,
      "options": [
        { "label": "人看的文本", "value": "程序用的实际值", "recommended": true },
        { "label": "…", "description": "补充说明（可选；渲染为该行的灰色副文案）" }
      ]
    }
  ]
}
```

卡片版式（单选 / 多选**外观一致**，区别只有能否多选）：

```
请回答以下问题                                   1 / 4
──────────────────────────────────────────────────────
今天午餐想吃什么？   (单选)
 A   热辣过瘾，适合多人聚餐
 B   滋滋冒油，配啤酒绝了   [推荐]      ← 整行高亮 = 当前选中
 C   健康轻食，适合减脂
 D   简单快捷，中式经典
 E   [ 输入自定义答案        ]        ← allow_input 时的最后一行（行内输入框）
──────────────────────────────────────────────────────
[✏️ 推荐选项]                     [取消]  [继续]
```

- 选项行行首是 `A/B/C/D…` 字母序号，整行可点、整行高亮表示选中（不再有「小 chip」与「原生复选框」两套外观）。
- 卡片聚焦时支持键盘 `A`–`F` 直接选中对应行。
- 底部左「推荐选项」（见 §5）、右「取消 / 上一步 / 继续」；**当前题未作答时「继续」置灰不可点**。

---

## 交互原语：只有「问题 + options」

`request_user_action` 不再有平铺的 action 与 `type` 枚举（confirm/select/input/multi_select 均已废弃）。
**只有一种交互原语：问题 + 一组 options。** 不同的交互形态只是同一原语的不同字段组合。

### 1. 单选（`multi` 缺省 / `false`）

默认交互形态。选项行互斥：点选即选中并自动进入下一题（末题除外）；最后一题作答后点底部「继续」统一回传（末题按钮即提交语义）。该项回传用所选 `value`（未填则回传 `label`）。

| 字段 | 必填 | 说明 |
|------|------|------|
| header | ✅ | 短标签（≤4 字），同批内唯一，作为该题答案的回传键 |
| question | ✅ | 问题全文，说明需要用户做什么决定 |
| options | ✅ | 2-5 项可选答案 |
| multi | ❌ | `false` 或缺省 = 单选 |
| allow_input | ❌ | 缺省/`true` = 选项列表末尾附一行行内输入框（`placeholder` 为「输入自定义答案」，点它即可输入；单选填了自定义即以其覆盖预设）；仅 `false`（options 已穷尽）才隐藏 |
| options[].recommended | ❌ | `true` = 该选项为推荐项（每题至多一个）；前端加「推荐」角标并让左下角「推荐选项」按钮可点 |

### 2. 多选（`multi: true`）

用户可勾选多项，勾选后点底部「继续」进入下一题（整批在最后一题统一提交）。回传勾选项按 `value`（缺省 `label`）以 `", "` 连接；`allow_input` 填了自定义文本会并入其后。

| 字段 | 必填 | 说明 |
|------|------|------|
| header | ✅ | 短标签，作为该题答案的回传键 |
| question | ✅ | 问题全文 |
| options | ✅ | 2-5 项，逐项可勾选 |
| multi | ✅ | `true` = 可多选 |
| allow_input | ❌ | 缺省/`true` = 选项列表末尾附一行行内输入框（`placeholder` 为「输入自定义答案」，点它即可输入；单选填了自定义即以其覆盖预设）；仅 `false`（options 已穷尽）才隐藏 |
| options[].recommended | ❌ | `true` = 该选项为推荐项（每题至多一个）；前端加「推荐」角标并让左下角「推荐选项」按钮可点 |

> 多选由用户勾选并点「继续」，整批在最后一题统一提交；**无需**为它额外构造确认/提交问题。

### 3. 确认 / 跳过 / 执行（是-否或决定类）

「确认 / 跳过 / 执行」这类是/否或决定：做成**单选问题**的 options。
旧 `confirm` 概念消失。

```json
{
  "questions": [
    {
      "header": "动作",
      "question": "是否执行？",
      "options": [
        { "label": "执行", "value": "run" },
        { "label": "暂不执行", "value": "hold" }
      ]
    }
  ]
}
```

### 4. 自定义答案兜底（默认开启）

reasonix 交互**每题都带**「自定义答案」，这里同样默认开启（`allow_input` 缺省即 `true`）。
前端把它渲染成**选项列表的最后一行**（行首字母序号顺延在预设选项之后，如第 5 行即 `E`）：这一行的内容区就是一个**常驻的行内输入框**，`placeholder` 即「输入自定义答案」——点它就能直接打字，**没有展开/折叠两态**，也没有「清除 / 完成」按钮（清空即手动删除）。输入非空时整行按选中态高亮。

取值规则：
- 单选：只要用户填了自定义文本，就**以该文本覆盖所选预设**作为该题答案（单选仍为单值；开始输入自定义内容会自动取消预设选中——只是点一下输入框不打字不算作答；反过来再点某个预设选项也会清掉自定义文本）。
- 多选：自定义文本并入勾选项之后，以 `", "` 连接。
- 完全没填：视为未作答该问——此时底部「继续」置灰，**无法空题跳过**（用户只能先作答，或点「上一步」/「取消」）。
- 用户取消整批：回传 `Cancel`（tool 结果为 `cancel` 标签 + 空串，与「提交但全部未作答」的空串可区分）。

旧 select 的「自定义输入 / allow_custom」与独立的 `input` 动作都并入这个字段。
仅当某问 options 已穷尽、确不需要用户自由补充时，才设 `allow_input: false` 隐藏该行。

### 5. 推荐项（`options[].recommended`，可选）

把你认为最合适的那个选项标 `recommended: true`——**每题至多一个**，没有把握就都不标。
前端据此在该选项行显示「推荐」角标，并让卡片左下角的「✏️ 推荐选项」按钮可点：用户点它即一键采纳该推荐（单选 = 选中并推进、多选 = 勾上该行）。本题没有任何推荐项时该按钮置灰。

```json
{
  "questions": [
    {
      "header": "路径",
      "question": "目标路径是哪个？",
      "allow_input": true,
      "options": [
        { "label": "当前目录", "value": "." },
        { "label": "上一级目录", "value": ".." }
      ]
    }
  ]
}
```

---

## 批量与独立性

- 一次 `request_user_action` 可携带 **1..≤4 个彼此独立、无依赖的并列问题**，一次收齐，减少打断。
- 若问题之间存在从属 / 依赖（A 的答案决定是否问 B），**必须拆开分次调用**，不得硬塞进同一批：
  先问 A → 等回传 → 再据 A 的答案决定是否问 B。

---

## 参数生成原则

1. **header 短标签**：≤4 字，同批内唯一，作为该题答案的回传键，前缀/命名体现问题归属。
   - `类型`、`路径`、`时间范围`、`字段`、`参数`
2. **question 写法**：写清需要用户做什么决定，用户无需额外解释。
   - ✅ `"是否执行？"` / `"从结果中勾选需要输出的字段"`
   - ❌ `"请点击下方按钮确认生成执行计划的整个流程并同时选择若干输出字段…"`
3. **options[].label 给人看、options[].value 给程序用**：当选项有对应的实际数据值时，务必填充 `value`，让系统直接取用、无需二次解析。
   - ✅ `{ "label": "城市：北京", "value": "北京" }`
   - ❌ 把机器值硬塞进 `label`，或期望回传 `label` 之外的加工值
4. **每问 2-5 项**；推荐项用 `recommended: true` 标注（每题至多一个），不要再靠“放第一个”表达推荐。
5. **value 内容限制**：多选回传用 `", "` 分隔，避免单选项的 `value` / `label` 内含逗号 `,`，否则会被解析截断。

---

## 回传（用户作答结果）

用户作答后回给 LLM 的 tool 结果形如**多行文本 `header => answer`**（每问一行）；未作答的问题该行省略（新交互下「继续」在未作答时置灰，正常不会出现；仅在取消整批等边界场景出现）。

```
类型 => CSV
路径 => /tmp/output.csv
```

LLM 读取时按 `header` 对应到具体问题：

- 单选：该行的 `answer` 是被点选项的 `value`（缺省 `label`）；若该问 `allow_input` 且用户填了自定义文本，则 `answer` = 该自定义文本（覆盖预设，单选仍单值）
- 多选：`answer` 是勾选项按 `", "` 连接后的文本；用户填了自定义文本则并入其后
- 用户取消整批：tool 结果为 `cancel` 标签 + 空串（与「提交但全部未作答」的空串可区分）

---

## 完整示例

### 单一批次多问题：参数固化的确认识别

AI 识别到 "在北京搜索 v2.1.0 版本 Rust 项目" 中的可参数化值，用一次调用收齐确认：

```json
{
  "message": "识别到以下可参数化的动态值，确认需要固化的参数：",
  "questions": [
    {
      "header": "参数",
      "question": "勾选需要固化的参数：",
      "multi": true,
      "options": [
        { "label": "城市", "value": "北京" },
        { "label": "版本", "value": "v2.1.0" },
        { "label": "搜索关键词", "value": "Rust" }
      ]
    },
    {
      "header": "动作",
      "question": "是否固化所选参数并继续？",
      "options": [
        { "label": "确认固化", "value": "confirm" },
        { "label": "跳过，直接执行", "value": "skip" }
      ]
    }
  ]
}
```

用户勾选"城市"和"版本" → 在"动作"选"确认固化" → 回传：

```
参数 => 北京, v2.1.0
动作 => confirm
```

系统本地解析得到：参数 `city=北京`、`version=v2.1.0`，无需 AI 再次参与。

### 单选问题（处理方式选择）

```json
{
  "questions": [
    {
      "header": "格式",
      "question": "你想怎么处理这个数据？",
      "options": [
        { "label": "导出 CSV", "value": "csv", "description": "适合 Excel 打开", "recommended": true },
        { "label": "导出 JSON", "value": "json", "description": "适合程序读取" },
        { "label": "屏幕打印", "value": "screen", "description": "直接显示结果" }
      ]
    }
  ]
}
```

用户点击"导出 CSV" → 回传 `格式 => csv`

### 单选问题 + 自由输入兜底

```json
{
  "questions": [
    {
      "header": "路径",
      "question": "请提供目标文件路径（或直接选用当前目录）：",
      "allow_input": true,
      "options": [
        { "label": "使用当前目录", "value": "." }
      ]
    }
  ]
}
```

用户输入 `/tmp/output.csv` → 回传 `路径 => /tmp/output.csv`；
用户点"使用当前目录" → 回传 `路径 => .`。

---

## 与 AI 产出内容的分界

`request_user_action` 的问题答案回传是**短字符串**，适合承载：确认信号、被选项的 `value`、用户自由输入。
当用户需要确认的对象是 **AI 展示的一段内容**（数据格式、JSON、计划文本等）时：

- 内容由 AI 在**对话消息**中产出，**不要**塞进选项的 `value`，也不要期望回传携带大段内容
- 调用方在卡片挂起时已持有对话历史快照，可在用户确认后从快照中提取内容，或由 AI 在后续对话中继续产出
- 示例：AI 先输出 `{"name":"test"}` 再弹"可以/取消"卡片——用户点"可以"后，回传只是"动作 => 可以"，JSON 应从对话快照中提取
