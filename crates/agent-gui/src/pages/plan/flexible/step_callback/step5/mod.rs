//! `flexible_step5` 结果链的组装点：前置分析 + 定稿登记两环。
//!
//! 实现在同目录 `step5_callback.rs`（[`Step5Callback`]，含本 step 的常量、整段模板副本的
//! 落库逻辑与单测）；解析、守门与对外定稿在 [`super::prelude`]；零策略工具在
//! [`super::commit`] 与 [`super::analysis`]。
...
use step5_callback::{Step5Callback, AGENT, OK_STATUS};
...
pub fn create_step5_callback(
    ...
    SubAgentResultChain::new(vec![Arc::new(Step5Callback::new(plan_id, service))])
