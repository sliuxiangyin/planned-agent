//! `flexible_step5` 的定稿登记回调：模板编译成功后把状态推进到 `templated`。
//!
...
//! 也不清任何下游产物 —— 故不用 [`super::super::commit::build_patch`]，就地构造补丁；
//! 写库与交接仍走公共工具，解析与守门在 [`super::super::prelude`]，链组装在 [`super`]。
...
pub(super) const AGENT: &str = "flexible_step5";
...
pub(super) struct Step5Callback {
