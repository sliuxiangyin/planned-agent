//! `flexible_plan` 结果链的组装点：前置分析 → 定稿登记。
//!
//! # 链上两环
//!
//! 1. **默认前置分析**（[`FlexibleStepPrelude`]）：解析输出、定位会话、判定定稿，并定稿对外文本；
//!    解析失败 → 要求重新输出，非定稿 → 直接终止链（定稿登记不执行）。
//! 2. **定稿登记**（[`PlanCallback`]）：写 `steps`、推进 `planned`。
//!
//! 实现在同目录 `plan_callback.rs`（定稿登记）；解析、守门与对外定稿在 [`super::prelude`]。
//!
...
use plan_callback::{PlanCallback, AGENT, OK_STATUS};

/// 创建 `flexible_plan` 结果链（方便传给 `register_sub_agent`）。
///
...
pub fn create_plan_callback(
    ...
    SubAgentResultChain::new(vec![Arc::new(PlanCallback::new(plan_id, service))])
