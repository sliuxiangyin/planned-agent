//! `flexible_step4` 结果链的组装点：前置分析 + 定稿登记两环。
//!
//! 实现在同目录 `step4_callback.rs`（[`Step4Callback`]，含本 step 的常量与单测）；
...
use step4_callback::{Step4Callback, AGENT, OK_STATUS};
...
pub fn create_step4_callback(
    ...
    SubAgentResultChain::new(vec![Arc::new(Step4Callback::new(plan_id, service))])
