//! `flexible_step1` 的组装点：结果链（前置分析 + 定稿登记两环）+ 启动前注入。
//!
...
///
/// 重做 step1 时 state 里仍是上一版基线 ⇒ 注入的也是它：基线的来源始终只有 state 一处，
/// 与 `flexible_step1.toml` 的输入说明一致（不再依赖协调器 LLM 的对话记忆）。
pub(super) const INJECT_MAPPING: &[(&str, &str)] = &[
    ("task_definition", "previous_task_definition"),
];

/// 创建 `flexible_step1` 的启动前注入回调（`StateInjectCallback` 的薄包装：
/// 映射留在本 step，改映射不必翻注册点）。
pub fn create_step1_inject(
    plan_id: String,
    service: Arc<PlansFlexibleService>,
...
mod tests {
    ...
    #[test]
    fn inject_mapping_matches_step1_input_contract() {
        assert_eq!(
            INJECT_MAPPING,
            &[("task_definition", "previous_task_definition")]
        );
