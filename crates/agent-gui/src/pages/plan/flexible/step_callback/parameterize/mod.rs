//! `flexible_parameterize` 结果链的组装点：前置分析 → 定稿登记。
//!
//! # 链上两环
//!
...
use parameterize_callback::{ParameterizeCallback, AGENT, OK_STATUS};

/// 创建 `flexible_parameterize` 结果链（方便传给 `register_sub_agent`）。
///
/// 链上两环：**前置分析**（解析 + 守门 + 定稿对外文本）+ 本 step 的定稿登记回调。
pub fn create_parameterize_callback(
    plan_id: String,
    service: Arc<PlansFlexibleService>,
) -> SubAgentResultChain {
    SubAgentResultChain::new(vec![
        // 定稿登记：写 inputs 与占位后的 steps、推进 parameterized
        Arc::new(ParameterizeCallback::new(plan_id, service)),
    ])
    // 前置分析：解析 + 守门 + 定稿对外文本
...
///
/// - `task_definition` ← `flexible_state.products.task_definition`（需求澄清定稿产物）。
...
/// 创建 `flexible_parameterize` 的启动前注入回调（映射留在本 step）。
pub fn create_parameterize_inject(
    plan_id: String,
    service: Arc<PlansFlexibleService>,
...
mod tests {
    ...
    #[test]
    fn inject_mapping_matches_parameterize_input_contract() {
        assert_eq!(
            INJECT_MAPPING,
