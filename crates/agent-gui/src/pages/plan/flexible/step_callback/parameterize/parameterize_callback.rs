//! `flexible_parameterize` 的定稿登记回调：参数化成功后写 `inputs` 与占位后的 `steps`，并推进 `parameterized`。
//!
//! 归属定位：从 `SubAgentCallContext.arguments` 读取 `host_session_id`
//! （见 `docs/chat-flexible-回调会话归属设计.md`）。
//!
//! 定稿判定（与 `flexible_parameterize.toml` 的输出契约一致）：`{"status":"success", ...}` 才算定稿；
...
pub(super) const AGENT: &str = "flexible_parameterize";
...
#[async_trait]
impl SubAgentResultCallback for ParameterizeCallback {
    async fn on_result(&self, call: &SubAgentCall<'_>) -> ResultDecision {
        ...
        .await
        {
            ...
            // 「新轨迹 + 未推进的 current_step」的中间态 —— 协调器重跑参数化时会覆盖，无需补偿。
