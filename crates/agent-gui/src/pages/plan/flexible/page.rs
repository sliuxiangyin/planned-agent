...
use crate::components::chat::{
    chat_flow::{
        ChatContext, ChatMessage, ChatStorage, DummyStorage,
        ChatSignals, PendingUI,
    },
    ChatPanel,
};
    ...
    // ── 初始化 ChatStorage + ChatContext，并加载历史消息（仅首次） ──
    {
        ...
        use_effect(move || {
            if let Some(repo) = storage_repo(storage_resource, |ctx| ctx.chat_message_repo()) {
                let storage: Arc<dyn ChatStorage> = Arc::new(ChatMessageStorage::new(repo));
                let ctx = ChatContext {
        ...
        div { class: "flexible-page",
                    ...
                    Button {
                        ...
                        onclick: move |_| {
                            ...
                            tokio::spawn(async move {
                                storage.delete_messages(&pid).await;
                            });
                        },
                        Trash2 { size: "16" }
                    }
            ...
            ChatPanel {
                ...
                on_user_action: move |(action, choice, pending)| {
                    chat.handle_user_action(action, choice, pending, chat_service_signal);
                },
