...
/// - **变体策略**：默认 48px 高度；通过传入 `class: "page-header--nested"` 切换为 40px 嵌入版
/// - **API 灵活**：`on_back` / `actions` / `back_label` 均为 Option，调用方按需组合
///
/// ## Props
///
/// - `title`：页面标题（h1）
...
pub fn PageHeader(
    ...
    // 合并默认 class 与调用方传入的 class
    let merged_class = match &class {
        Some(extra) if !extra.is_empty() => format!("page-header {}", extra),
        _ => "page-header".to_string(),
    };
