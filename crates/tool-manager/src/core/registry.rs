...
use crate::core::validator::ToolValidator;
// McpManagerTrait 已下沉到 core；这里通过 adapter 模块重新导出
...
impl ToolRegistry {
    ...
    /// 仅更新 resume 信号（不更新 session），内部使用
    pub fn upsert_resume_signal(&self, session_id: String, resume_tx: tokio::sync::oneshot::Sender<Value>) {
        self.sub_agent_sessions.upsert_resume_signal(session_id, resume_tx);
    }
